...
지키려는 규칙:
    ...
    2. 조회 라우트는 쓰기 세션(`get_routed_db_session`)을 쓰지 않는다   (P4-3)
...
# 쓰기 세션(get_routed_db_session)으로 서비스를 구성하는 의존성 전부.
_WRITE_DEPENDENCIES = {
    get_auth_service,
    get_blog_service,
    get_reply_service,
...
def _api_routes() -> list[APIRoute]:
    ...
    """
    session_deps = {get_routed_db_session, get_read_only_db_session}
...
def test_read_routes_do_not_use_writer_session():
    ...
    offenders = [
        _name(route)
        for route in _read_routes()
        if get_routed_db_session in _dependency_calls(route)
        and _name(route) not in _WRITER_SESSION_BY_DESIGN
    ]

    assert not offenders, (
        f"조회 라우트가 쓰기 세션(get_routed_db_session)을 사용함: {sorted(offenders)}. "
