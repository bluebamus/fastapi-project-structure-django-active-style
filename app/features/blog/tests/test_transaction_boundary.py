...
커밋은 쓰기 핸들러 본문이 `await service.commit()` 으로 한 번 한다(P1-3). 예전에는
`get_blog_service` 가 `yield` 이후 커밋했고, 그 경계는 FastAPI 의 yield dependency
종료 시점에 의존해 버전을 올릴 때 조용히 바뀌었다. 여기서 네 가지 성질을 고정한다.
...
이 테스트가 정확히 그 판정을 위해 존재하고, P1-3 적용 뒤 현재는 통과한다. 설계 근거는
`docs/guides/DEVELOPMENT.md` §8 (트랜잭션 경계 — 커밋은 핸들러 본문).
...
from app.core.db.session import Base, get_read_only_db_session, get_routed_db_session
...
async def tx_client():
    ...
    app.dependency_overrides[get_routed_db_session] = _override_get_session
