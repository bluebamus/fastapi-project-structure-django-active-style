...
| R-014 | Action 을 commit SHA 로 고정(F-033)한 결과 **자동 갱신이 사라졌다**. 고정한 `actions/checkout` v4 · `astral-sh/setup-uv` v5 는 Node.js 20 런타임을 대상으로 하며, GitHub 는 이를 폐기 예고하고 현재는 Node 24 로 강제 실행 중이다(2026-08-19 CI 주석). | LOW | SHA 고정은 "검토한 커밋만 우리 CI 안에서 돈다" 를 얻는 대신 갱신을 수동 작업으로 만드는 거래다. 그 거래를 의도적으로 택했다. 현재는 경고이며 실행은 성공한다. | 2026-08-19 | GitHub 가 Node 20 강제 실행을 중단하거나 Action 에 보안 권고가 나오면, 새 태그의 SHA 를 확인해 교체한다. |
