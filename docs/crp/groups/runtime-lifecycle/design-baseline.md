...
| REQ-014 | 2026-09-21 | "남은 작업, 고려해야 하는 사항 정리해줘" → "진행해줘" | `get_writer_db_session()`·`get_read_only_db_session()` 의 `try/except: rollback; raise` 를 지운다. 같은 일을 `async with` 가 이미 하고 있어 죽은 코드다. | Active | ADR-011 |
...
| ADR-011 | 2026-09-21 | 로깅 부수효과가 **없는** 세션 제너레이터(`get_writer_db_session`·`get_read_only_db_session`)는 `yield session` 한 줄만 두고, 정리는 `async with` 에 맡긴다. 로그를 남기는 `get_routed_db_session`·`get_background_db_session` 은 except 블록을 유지한다. | SQLAlchemy 2.0.51 의 `AsyncSession.__aexit__` 는 `close()` 를 부르고, `close()` 는 `_close_impl` 에서 `if self.is_active: self._connection_rollback_impl()` 로 ROLLBACK 을 이미 보낸다. 엔진 이벤트 실측에서 명시 rollback 이 있으나 없으나 ROLLBACK 은 정확히 1회였다. 게다가 `except Exception` 은 `asyncio.CancelledError`(BaseException 상속 — 클라이언트가 응답 전에 끊은 경우)를 **못 잡는데** `__aexit__` 는 잡는다. 변이 검증에서도 `close()` 를 없앴을 때 실패한 것은 cancelled 경로뿐이었다 — 지우는 쪽이 예외 안전성이 더 넓다. | Accepted | — |
...
- v0.6 (2026-09-21): REQ-014 · ADR-011 추가 — writer·read-only 세션 제너레이터의 죽은
  `try/except rollback` 을 지우고, 예외 경로의 ROLLBACK 1회를 엔진 이벤트로 고정하는
  테스트를 남겼다. ARCHITECTURE §세션 설명도 "모든 generator 가 rollback 한다" 에서
  "close() 가 ROLLBACK 한다" 로 바로잡았다.
