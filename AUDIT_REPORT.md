...
| 적용한 수정 | ruff format(57파일) · mypy 49→**3** · bandit B104 · eager-loading(L-2) · 미들웨어 순서(L-4) · 믹스인 채택(L-3) |
| 최종 bandit 감사 | High 0 / Medium 0 / Low 68(전부 테스트 assert 오탐) → **비테스트 실이슈 0** (§4-b) |
| 보류(소유자 결정) | 1건 (BaseRepository 미사용 표면 축소 여부) + nginx 계층 제외 항목 |
| 커밋 | 6개 수정 커밋 (`0220eba`,`8bc17be`,`cb384b2`,`9f11513`,`4d39216`,`7880165`) + 문서 |
| 설치 도구 | **bandit** (uvx 임시 실행, 의존성 미영구화). ruff·mypy 는 기존 dev 의존성. |

> **후속 처리(2회)**: 소유자 지시로 1차 보류를 순차 처리 — L-2(중첩 eager-loading+회귀테스트)·L-4(미들웨어 순서), 이어서 L-3(믹스인 채택) 적용 완료. **HOST 노출은 nginx 계층 항목으로 제외**. 마지막에 최종 bandit 감사 수행(비테스트 실이슈 0). 잔여 결정은 `BaseRepository` 미사용 표면 축소 여부(파괴적이라 소유자 판단) 뿐.
...
> **처리 경과**: 1차 §4 4건 중 **L-2(중첩 eager-loading)·L-4(미들웨어 순서)** 수정 완료. 2차로 **L-3(미사용 믹스인)** 채택 완료. **HOST 노출(구 1번)** 은 소유자 지시에 따라 **nginx 계층 항목으로 제외**. 아래는 남은 결정 사항.

1. **[템플릿 방향성 — 미사용 BaseRepository 표면]** — `BaseRepository` 의 eager-loading/bulk/upsert/partial/join/batch 다수가 앱 내 미사용(앱 모델에 relationship 없음). L-2 수정으로 중첩 로더는 정상 동작하고 회귀 테스트로 관계 예시도 확보됐으나, 도메인 모델에는 여전히 소비처가 없음.
   - 권고(택1): (a) 재사용 스캐폴딩으로 유지(현행) — 회귀 테스트가 사용 예시 역할, 또는 (b) 표면을 실제 사용 메서드 중심으로 축소. — 템플릿 방향성에 대한 소유자 판단. **파괴적 축소는 자동 미적용**.
   - 참고: 미사용 믹스인(L-3)은 모델이 채택하도록 정리 완료.
